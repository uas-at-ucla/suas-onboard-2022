...
# Generate waypoints from drop locations; waypoints seperated by a home point.
# Note: Altitudes are given in feet MSL(global)


def generate_drop_list(filename):
    # Waypoints format: [lat, lon, alt]
    home = [38.31442311312976, -76.54522971451763, 180]  # Sample return point
    waypoint_list = []
    with open(filename) as f:
        for line in f:
            waypoint = line.strip().split(",")
    ...
    for waypoint in waypoint_list:
        ...
        waypoint[2] *= METERS_PER_FEET
        waypoint[2] -= AIRFIELD_ALT
        waypoint[2] -= 5  # Lower altitude by 5 meters for drop
    return waypoint_list
...
mission_add_waypoints(generate_drop_list("dropLocations.txt"))
