...
def binary_dilation(image):
    #inspired by https://github.com/danvk/oldnyc/blob/master/ocr/tess/crop_morphology.py
    kernal_size = int(os.environ.get("DILATION_KERNAL_SIZE"))
    middle_index = int((kernal_size-1)/2)
    kernal = np.ones((kernal_size, kernal_size), np.uint8)
    """Construct a + shape kernal for binary dilation"""
    # kernal[middle_index, :] = 1
    # kernal[:, middle_index] = 1
    dilated_image = image
    #TODO: find better system than repeated dilating an image, ALSO CHANGE MAGIC NUMBER
...
def crop_image(image):
    ...
    center, size, angle = bounding_rectangle[0], bounding_rectangle[1], bounding_rectangle[2]
